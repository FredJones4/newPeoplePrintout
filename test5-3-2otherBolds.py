...
def replace_placeholder(page, placeholder, replacement_text, fontsize=12, color=(0, 0, 0), bold=False):
    for inst in page.search_for(placeholder):
        ...
        if bold:
            # Use 200% of the original font size for bold text
            ...
            y_position = inst.y0 + (inst.height / 2) + (bold_fontsize / 4)  # Adjusted for larger font
            page.insert_text((x_center, y_position), replacement_text, fontsize=bold_fontsize, fontname=fontname,
                             color=color)
        else:
